...
func setup(withGenesis bool, invCheckPeriod uint, dir string) (*app.App, app.GenesisState) {
	db := tmdb.NewMemDB()
	encoding := cosmoscmd.MakeEncodingConfig(app.ModuleBasics)
	a := app.New(log.NewNopLogger(), db, nil, true,
		map[int64]bool{}, dir, invCheckPeriod, encoding, simapp.EmptyAppOptions{})
	if withGenesis {
		return a.(*app.App), app.NewDefaultGenesisState(encoding.Marshaler)
	}
	return a.(*app.App), app.GenesisState{}
}
